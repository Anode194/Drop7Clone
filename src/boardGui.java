...
public class boardGui extends JFrame {
	...
	public boardGui() {
		...
		JButton[] btnArray = new JButton[49];
		
		for ( int x = 0; x < 49; x++) {
			
			btnArray[x] = new JButton();
			btnArray[x].setBorder(new LineBorder(new Color(0, 0, 0),2));
			btnArray[x].setBackground(Color.DARK_GRAY);
			btnArray[x].setForeground(Color.RED);
			btnArray[x].setOpaque(true);
			btnArray[x].setText(String.format("%d", x));
			panel.add(btnArray[x]);
		}
	}
}
